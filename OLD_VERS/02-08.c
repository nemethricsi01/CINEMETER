...
uint16_t read_distance() {
	...
	static uint16_t prev_distance_mm = 0;   // Initialize to a safe value
	...
	if (usart_rx_buf[0] == HEADER && usart_rx_buf[1] == HEADER) {
		...
		if (distance_mm >= 2000) { // check if difference is less than or equal to 1500mm
			if (switch_counter < max_counter) {
				switch_counter++;
			} else {
				switch_counter = 0; // Reset the switch counter
				// Führen Sie hier Ihre Sensorenwechsel-Logik aus
				...
				} else if (switch_counter > 0) { // Falls wir in der Mitte des Zählvorgangs sind und die Bedingung nicht mehr erfüllt ist, setzen wir den Zähler zurück
					switch_counter = 0;
				} else {
					prev_distance_mm = distance_mm; // Update the previous distance
...
uint32_t get_distance() {
	...
	if (distance_mm >= 1000) { // change to 3000, because now it is in mm
//		read_data_uart1();
		int integer_part = temp_d / 100;
		int fractional_part = (temp_d % 100);
		sprintf(tx_buffer, "%d.%02d\r\n", integer_part, fractional_part);

//			HAL_UART_Transmit(&huart2, (uint16_t*)tx_buffer, strlen(tx_buffer), HAL_MAX_DELAY);
		show_Display();
...
void show_Display() {
	...
	ssd1306_WriteString("SENS: AUTO", Font_7x10, White);
