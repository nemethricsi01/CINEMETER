...
#define BTN_SELECT_PIN   GPIO_PIN_11
#define BTN_PREVIOUS_PIN GPIO_PIN_12
...
#define MENU_ITEM_COUNT (sizeof(menuItems) / sizeof(MenuItem))
...
static uint32_t menuBtnPressTime = 270;
const uint32_t longPressDelay = 1000;
...
{		ssd1306_Fill(Black);
    ...
    {
        ssd1306_SetCursor(40, i * 11);
        ssd1306_WriteString(menuItems[i].text, Font_7x10, (i == currentMenuItem) ? White : Black);
    }
    ssd1306_UpdateScreen();
}
void updateMenuState(void)
{
    // Handle menu activation/deactivation regardless of menu state
    if (HAL_GPIO_ReadPin(GPIOC, BTN_SELECT_PIN) == GPIO_PIN_SET) {
        if (!menuActive) {
            menuActive = 1;
            menuBtnPressTime = HAL_GetTick();
        } else {
            if (HAL_GetTick() - menuBtnPressTime > longPressDelay) {
                menuActive = 0;
            }
        }
    ...
    // Handle other buttons only if menu is active
    if (menuActive && HAL_GetTick() - lastDebounceTime > debounceDelay) {
        if (HAL_GPIO_ReadPin(GPIOC, BTN_NEXT_PIN) == GPIO_PIN_SET) {
            currentMenuItem = (currentMenuItem + 1) % MENU_ITEM_COUNT;
            lastDebounceTime = HAL_GetTick();
        } else if (HAL_GPIO_ReadPin(GPIOC, BTN_PREVIOUS_PIN) == GPIO_PIN_SET) {
            currentMenuItem = (currentMenuItem == 0) ? MENU_ITEM_COUNT - 1 : currentMenuItem - 1;
            lastDebounceTime = HAL_GetTick();
        } else if (HAL_GPIO_ReadPin(GPIOC, BTN_SELECT_PIN) == GPIO_PIN_SET) {
            if (HAL_GetTick() - menuBtnPressTime > longPressDelay) {
                menuItems[currentMenuItem].state = (menuItems[currentMenuItem].state == STATE1) ? STATE2 : STATE1;
                save_to_flash();
            }
        }
    }
...
void RS232() {
	...
	if (__HAL_TIM_GET_COUNTER(&htim10) - timer_val >= 100) {
