...
#define FLASH_USER_ADDR 0x0803FFFF // Start address of the last sector (Sector 7, 16 Kbytes)
...
int offset = 0;
int menuOpen = 0;
int pc11_pressed_count = 0;

void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
    if (GPIO_Pin == GPIO_PIN_10) { // Button PC10 pressed
        offset++;
    } else if (GPIO_Pin == GPIO_PIN_12) { // Button PC12 pressed
        offset--;
    } else if (GPIO_Pin == GPIO_PIN_11) { // Button PC11 pressed
        pc11_pressed_count++;

        if(pc11_pressed_count == 2) {
            pc11_pressed_count = 0; // Reset the counter

            if(menuOpen) {
                menuOpen = 0; // Close the menu
                printf("Menu closed\n");
            } else {
                menuOpen = 1; // Open the menu
                printf("Menu opened\n");
                HAL_FLASH_Unlock(); // Unlock the FLASH control register access
                ...
                NbOfPages = (FLASH_USER_END_ADDR - FLASH_USER_START_ADDR) / FLASH_PAGE_SIZE;
                ...
                if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, FLASH_USER_START_ADDR, offset) != HAL_OK) {
...
int readButtonState(uint16_t buttonPin) {
	if (HAL_GetTick() - lastButtonPressTime < BUTTON_DEBOUNCE_DELAY) {
		return FALSE;
	}

	if (HAL_GPIO_ReadPin(GPIOC, buttonPin) == GPIO_PIN_SET) {
...
//                      writeData();
		...
		button_state = HAL_GPIO_ReadPin(BUTTON_2_GPIO_Port, BUTTON_SELECT_PIN);

		// Check for a button press
		if (button_state == GPIO_PIN_SET
				&& prev_button_state == GPIO_PIN_RESET) {
		...
		if (button_state == GPIO_PIN_SET && prev_button_state == GPIO_PIN_SET
				&& button_press_count) {
