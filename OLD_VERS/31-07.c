...
//	HAL_Delay(10);
	if(distance_mm > 3000) { // change to 3000, because now it is in mm
		...
		return tx_buffer;
		   ...
		   snprintf(dist_buffer, sizeof(dist_buffer), "%d.%02d", distance_mm/1000, (distance_mm%1000)/10);
		return dist_buffer;
	}
...
// CALCULATE DISTANCE FROM THE TWO SENSORS
// This function reads the data from the VL53L1 sensor
uint16_t read_data_VL53L1() {
	uint16_t distance_mm = TOF_GetDistance(&sensor1);

	 HAL_UART_Transmit(&huart2, (uint8_t*)distance_mm, strlen(distance_mm), HAL_MAX_DELAY);

	return distance_mm;

}

// This function reads the data from the uart1 sensor
uint16_t read_data_uart1() {
	...
	if (usart_rx_buf[0] == HEADER && usart_rx_buf[1] == HEADER) {
		...
		if (usart_rx_buf[8] == (checksum & 0xff)) {
			...
			if (dist != temp_d) {
				...
				if (potential_temp_d <= 4500 && potential_temp_d >= 002 && potential_temp_d != 65535) {
...
void print_distance() {
    ...
    // Convert the distance to a string, separating the integer part and the fractional part
