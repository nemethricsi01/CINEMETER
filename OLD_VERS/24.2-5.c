...
void saveSettingsToFlash(DeviceSettings *settings) {
	HAL_FLASH_Unlock();
	...
	if (HAL_FLASHEx_Erase(&EraseInitStruct, &SectorError) != HAL_OK) {
		...
		// You may add here some code to deal with this error.
		HAL_FLASH_Lock();
		return;
	}

	uint32_t Address = FLASH_USER_ADDR;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, Address,
			settings->currentMenuItem);
	Address += 4;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, Address, settings->meterSetting);
...
void loadSettingsFromFlash(DeviceSettings *settings) {
	uint32_t Address = FLASH_USER_ADDR;
	settings->currentMenuItem = *(__IO uint32_t*) Address;
	Address += 4;
	settings->meterSetting = *(__IO uint32_t*) Address;
}
